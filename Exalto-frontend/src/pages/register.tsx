    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex w-full flex-col items-center justify-center bg-white px-8 py-12 md:w-[55%]">
        <div className="w-full max-w-sm">
          ...
          {/* Heading */}
          <h1 className="mb-1 text-center text-3xl font-black text-[#251c18]">Create Account</h1>
          <p className="mb-6 text-center text-sm text-[#9a8a82]">Join Exalto and start ordering today</p>

          {/* Account type toggle */}
          <div className="mb-5 grid grid-cols-2 gap-1 rounded-xl border border-[#eee] bg-[#f9f6f3] p-1">
            {([
              { value: "individual", label: "customer", icon: User },
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            {/* Name */}
            <div>
              <label className="mb-1 block text-xs font-semibold text-[#3d291c]">Full Name *</label>
              <input required type="text" value={form.name} onChange={set("name")} placeholder="Your full name"
            ...
            {accountType === "business" && (
              <div>
                <label className="mb-1 block text-xs font-semibold text-[#3d291c]">Company Name *</label>
                <input required type="text" value={form.company} onChange={set("company")} placeholder="Your company name"
                  className="w-full rounded-lg border border-[#ddd] px-4 py-2.5 text-sm text-[#251c18] placeholder-[#c4b8b0] outline-none transition focus:border-[#c94708] focus:ring-2 focus:ring-[#c94708]/10" />
              </div>
            )}
            ...
            {/* Phone */}
            <div>
              <label className="mb-1 block text-xs font-semibold text-[#3d291c]">Phone Number</label>
            ...
            <div>
              ...
              <div className="relative">
                <input required minLength={8} type={showPassword ? "text" : "password"} value={form.password} onChange={set("password")} placeholder="Min. 8 characters"
            ...
            <div className="flex items-start gap-2">
              <input required id="terms" type="checkbox" className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 accent-[#c94708]" />
              <label htmlFor="terms" className="text-[11px] leading-5 text-[#9a8a82]">
                I agree to the{" "}
                <Link to="/terms" className="font-semibold text-[#c94708] hover:underline">Terms of Service</Link>
                {" "}and{" "}
                <Link to="/privacy-policy" className="font-semibold text-[#c94708] hover:underline">Privacy Policy</Link>
              </label>
            </div>
      ...
      <div className="hidden md:block md:w-[45%]">
        <img src={wineImage} alt="Exalto Products" className="h-full w-full object-cover" />
      </div>
